...
def get_tools(request):
    ...
    if id_str != "":
        ...
        if len(id_array) > 0:
            ...
            distinct_toolings = toolings.values('tooling', 'tnum', 'tool_name', 'holder_name', 'tip_name').distinct()
            page_size = 20
            paginator = Paginator(distinct_toolings, page_size)
            try:
                tooling_data = paginator.page(page_index)
                return Response(ToolingDataSerializer(tooling_data, many = True).data)
            except EmptyPage:
                return Response(data=[])
        else:
            return Response([])
...
def save_check_history(request):
    ...
    if serializer.is_valid():
        ...
        tool_check_history = ToolCheckHistory.objects.create(user = request.user, number = input_data["number"], 
            tooling = input_data["tooling"], program_info = input_data["onum"], started_at = input_data["started_at"])
...
def get_check_histories(request):
    ...
    if date != "":
        ...
        query_set = query_set.filter(created_at__year = date_obj.year, created_at__month = date_obj.month, created_at__day = date_obj.day)
    if user_id != "":
    ...
    try:
        ...
        return Response(ToolCheckHistorySerializer(history_data, many = True).data)
    except EmptyPage:
        return Response(data=[])
...
def get_check_detail(request):
    ...
    page_index = int(request.query_params.get("page", "1"))
    if detail_id > 0:
        details = ToolCheckDetail.objects.filter(history__id = detail_id).order_by('tooling', 'tnum')
        page_size = 20
        paginator = Paginator(details, page_size)
        try:
            detail_data = paginator.page(page_index)
            return Response(ToolCheckDetailSerializer(detail_data, many = True).data)
        except EmptyPage:
            return Response(data=[])     
    else:
        return Response(status = status.HTTP_400_BAD_REQUEST)
