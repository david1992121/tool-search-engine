...
class ToolCheckHistory(models.Model):
    id = models.AutoField("ID", db_column="ID", primary_key=True)
    user = models.ForeignKey(User, related_name="tool_histories", verbose_name="User", on_delete=models.CASCADE,
        db_column="UserID", to_field="id")
    number = models.IntegerField("No", db_column="CheckNo", default=1)
    tooling = models.CharField("Tooling", max_length=255, db_column="Tooling", null=True)
    program_info = models.CharField("ONum String", max_length=255, db_column="ONum String", null=True)
    started_at = models.DateTimeField('StartedAt', null=True, db_column="StartedAt")
    created_at = models.DateTimeField('EndedAt', auto_now_add=True, db_column="EndedAt")
...
class ToolCheckDetail(models.Model):
    id = models.AutoField("ID", db_column="ID", primary_key=True)
    history = models.ForeignKey(ToolCheckHistory, related_name="history_details", verbose_name="HistoryDetails", on_delete=models.CASCADE,
        db_column="HistoryID", to_field="id")
    tooling = models.CharField("Tooling", max_length=255, db_column="Tooling", null=True, blank=True)
    tnum = models.IntegerField("TNumber", db_column="TNumber", default = 0)
    tool_name = models.CharField("ToolName", max_length=255, db_column="ToolName", null=True, blank=True)
    holder_name = models.CharField("HolderName", max_length=255, db_column="HolderName", null=True, blank=True)
    tip_name = models.CharField("TipName", max_length=255, db_column="TipName", null=True, blank=True)
    exchange = models.BooleanField("Exchange", default=False, db_column="Exchange")
    correction = models.BooleanField("Correction", default=False, db_column="Correction")
